...
Berkson's Bias is a type of selection Bias that occurs when a study sample is not representative of a population due to the method that subjects are selected.

If selection is based on multiple factors (e.g., height and offensive skill), then analyzing only the selected group (e.g., NBA draftees) may falsely suggest a relationship between the variables that doesn’t hold in the broader population (e.g., all college players). This can lead to misleading conclusions about the impact of height on offensive performance.
